...
#   report_power -hier
#   set_max_delay
#   write -format db -output block_b.db
#
# Doesn't work quite the way I expect
#   NameDesign = block_b    Set variable ok, but how to concatenate?
#   write_rtl -format verilog -output block_b.vg


#===== Set: make sure you change design name elsewhere in this file
set NameDesign "block_b"
...
analyze -format verilog ./block_b.v
...
#===== Reports
write -format verilog -output block_b.vg -hierarchy $NameDesign

report_area               > block_b.area
report_cell               > block_b.cell
report_hierarchy          > block_b.hier
report_net                > block_b.net
report_power              > block_b.pow
report_timing -nworst 10  > block_b.tim
