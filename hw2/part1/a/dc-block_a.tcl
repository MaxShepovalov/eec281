...
#   report_power -hier
#   set_max_delay
#   write -format db -output block_a.db
#
# Doesn't work quite the way I expect
#   NameDesign = block_a    Set variable ok, but how to concatenate?
#   write_rtl -format verilog -output block_a.vg


#===== Set: make sure you change design name elsewhere in this file
set NameDesign "block_a"
...
analyze -format verilog ./block_a.v
...
#===== Reports
write -format verilog -output block_a.vg -hierarchy $NameDesign

report_area               > block_a.area
report_cell               > block_a.cell
report_hierarchy          > block_a.hier
report_net                > block_a.net
report_power              > block_a.pow
report_timing -nworst 10  > block_a.tim
