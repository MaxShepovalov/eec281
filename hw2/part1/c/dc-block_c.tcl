...
#   report_power -hier
#   set_max_delay
#   write -format db -output block_c.db
#
# Doesn't work quite the way I expect
#   NameDesign = block_c    Set variable ok, but how to concatenate?
#   write_rtl -format verilog -output block_c.vg


#===== Set: make sure you change design name elsewhere in this file
set NameDesign "block_c"
...
analyze -format verilog ./block_c.v
...
#===== Reports
write -format verilog -output block_c.vg -hierarchy $NameDesign

report_area               > block_c.area
report_cell               > block_c.cell
report_hierarchy          > block_c.hier
report_net                > block_c.net
report_power              > block_c.pow
report_timing -nworst 10  > block_c.tim
