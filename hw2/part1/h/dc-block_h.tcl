...
#   report_power -hier
#   set_max_delay
#   write -format db -output block_h.db
#
# Doesn't work quite the way I expect
#   NameDesign = block_h    Set variable ok, but how to concatenate?
#   write_rtl -format verilog -output block_h.vg


#===== Set: make sure you change design name elsewhere in this file
set NameDesign "block_h"
...
analyze -format verilog ./block_h.v
...
#===== Reports
write -format verilog -output block_h.vg -hierarchy $NameDesign

report_area               > block_h.area
report_cell               > block_h.cell
report_hierarchy          > block_h.hier
report_net                > block_h.net
report_power              > block_h.pow
report_timing -nworst 10  > block_h.tim
