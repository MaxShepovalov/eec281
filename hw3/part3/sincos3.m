...
fileID = fopen("sincos_small.txt","w");


for i = 0:last_val/16

    cs = round(cos(4 * pi * i / last_val) * (2^14));
    sn = round(sin(4 * pi * i / last_val) * (2^14));
    ...
    if (length(cs_b) ~= 15)
        ...
        fprintf(fileID, "    cos_mem[9'b%s] = 15'b%s; sin_mem[9'b%s] = 15'b%s;\n", dec2bin(i,9), dec2bin(0,15), dec2bin(i,9), sn_b);
    else
        fprintf(fileID, "    cos_mem[9'b%s] = 15'b%s; sin_mem[9'b%s] = 15'b%s;\n", dec2bin(i,9), cs_b, dec2bin(i,9), sn_b);
    end
end
